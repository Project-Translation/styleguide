...
https://project-translation.github.io/styleguide/i18n/zh-cn/go/best-practices
...
- [Go 提示 #42：编写测试存根](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
...
- [GoTip #89：何时使用规范状态代码作为错误](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [GoTip #48：错误哨兵值](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [GoTip #13：设计用于检查的错误](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
...
不要尝试根据错误的字符串形式来区分错误。（有关更多信息，请参见 [Go 提示 #13：设计用于检查的错误](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)。）
...
可以根据需要使用其他错误结构，例如包含错误代码和详细字符串的项目结构。[包 `status`][status] 是一种常见的封装；如果您选择这种方法（您没有义务这样做），请使用 [规范代码]。请参见 [Go 提示 #89：何时使用规范状态代码作为错误](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)，以了解使用状态代码是否是正确的选择。
...
[结构化错误](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip) 中（例如：
...
[良好的测试失败消息]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/decisions#useful-test-failures
...
[反对恐慌的决定]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/decisions#dont-panic
...
[GoTip #41: 识别函数调用参数]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
[GoTip #51: 配置模式]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
...
- 不清楚操作是只读还是修改的：
  ...
  为什么？查找键时的缓存命中会在内部修改 LRU 缓存。这种实现方式对所有读者可能并不明显。
...
[GoTip #110: 不要将退出与延迟混合]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
...
- [Go 提示 #106：错误命名约定](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示 #89：何时使用规范状态码作为错误](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
...
[Go技巧#3：基准测试Go代码]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
...
另见：
[Go 技巧#24：使用特定于用例的构造](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
...
[sentinels]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
[custom types]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
[aggregates errors]: https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip
...
例如，假设你要测试的代码（有时称为“被测系统”或 SUT）与实现[长运行操作]API 的后端进行交互。为了测试你的 SUT，使用连接到[OperationsServer]的[测试替身]（例如，模拟、存根或假对象）的真实[OperationsClient]。
...
**提示：** Go 1.14 引入了[`t.Cleanup`]函数，可用于注册在测试完成时运行的清理函数。该函数也适用于测试助手。有关简化测试助手的指导，请参见[GoTip #4: 清理您的测试](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)。
...
如[测试包中所记录](https://pkg.go.dev/testing#T)，从运行测试函数（或子测试）的 goroutine 之外调用`t.FailNow`、`t.Fatal`等是不正确的。如果你的测试启动了新的 goroutine，这些 goroutine 不能在其中调用这些函数。
...
**注意：** 有关更多讨论，请参见 [GoTip #29: 有效构建字符串](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)。
...
库不应强制其客户端使用依赖于[全局状态](https://en.wikipedia.org/wiki/Global_variable)的 API。建议它们不要暴露 API 或导出[包级别](https://go.dev/ref/spec#TopLevelDecl)变量，这些变量作为 API 的一部分控制所有客户端的行为。本节的其余部分将“全局”和“包级别状态”视为同义词。
...
- [Go 提示#5：瘦身您的客户端库](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#24：使用特定用例的构造](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#40：通过函数参数改进时间测试性](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#41：识别函数调用参数](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#44：通过结构字段改进时间测试性](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#80：依赖注入原则](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)

不支持显式依赖传递的 API 会随着客户端数量的增加变得脆弱：
...
默认情况下，Go 测试是按顺序执行的，因此上述测试按以下顺序运行：

1.  `TestEndToEnd`
2.  `TestRegression_NetworkUnavailability`，它覆盖了 cloudlogger 的默认值
3.  `TestRegression_InvalidUser`，它需要在`package sidecar`中注册的 cloudlogger 的默认值

这创建了一个依赖顺序的测试用例，这会破坏使用测试过滤器运行测试，并阻止测试并行运行或分片。

使用全局状态会带来一些对您和 API 客户端来说难以解决的问题：
...
- 如果多个客户端在同一个名称下注册插件，会发生什么情况？哪个会胜出，如果有的话？
  ...
  错误应该如何处理？如果代码发生恐慌或调用`log.Fatal`，这是否总是适合 API 被调用的所有地方？客户端能否在执行操作前验证其不会做坏事？

- 在程序启动阶段或生命周期的某些阶段，是否可以调用`Register`，以及在哪些阶段不能调用？

  如果在错误的时间调用`Register`会发生什么？客户端可以在[`func init`](https://go.dev/ref/spec#Package_initialization)中调用`Register`，在解析标志之前，或在`main`之后。函数被调用的阶段会影响错误处理。如果 API 的作者假设 API 仅在程序初始化期间被调用，而没有这样的要求，这种假设可能会促使作者设计错误处理以通过将 API 建模为类似`Must`的函数来[中止程序](best-practices#program-init)。对于可以在任何阶段使用的通用库函数，中止是不合适的。

- 如果客户端和设计者的并发需求不匹配怎么办？

另见：

- [Go 提示#36：封装包级状态](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#71：减少并行测试的不稳定性](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- [Go 提示#80：依赖注入原则](https://project-translation.github.io/styleguide/i18n/zh-cn/go/index.html#gotip)
- 错误处理：
...
全局状态对[Google 代码库的健康](guide.md#maintainability)有连锁效应。应以**极端审慎**的态度对待全局状态。
...
> **注意：** Google 代码库中的许多旧 API 不遵循此指导；事实上，一些 Go 标准库允许通过全局值进行配置。尽管如此，旧 API 违反此指导**[不应作为继续该模式的先例](guide#local-consistency)**。
>
> 今天投资于适当的 API 设计比以后为重新设计付出代价要好。
...
- 独立的测试用例通过全局状态相互干扰。
- API 的用户为了测试目的而倾向于交换或替换全局状态，特别是用[测试替身]（如存根、假对象、间谍或模拟）替换状态的任何部分。
- 用户在与全局状态交互时必须考虑特殊的排序要求：`func init`，标志是否已解析等。

如果避免了上述条件，则在以下**少数有限的情况下，这些 API 是安全的**，即当以下任何一项为真时：
...
> **注意：** > [边车进程](https://www.oreilly.com/library/view/designing-distributed-systems/9781491983638/ch02.html)
...
- 对`package image`的 API（例如`image.Decode`）的多次调用不会相互干扰，测试也是如此。唯一的例外是`image.RegisterFormat`，但这可以通过下面的几点得到缓解。
- 用户不太可能想要用[测试替身]替换解码器，因为 PNG 解码器体现了我们代码库偏好真实对象的情况。然而，如果解码器有状态地与操作系统资源（例如网络）交互，用户更有可能用测试替身替换解码器。
